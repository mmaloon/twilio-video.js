...
var nativeRTCPeerConnection = typeof RTCPeerConnection !== 'undefined' ? RTCPeerConnection : webkitRTCPeerConnection;
...
if (typeof adapter !== 'undefined' && typeof document !== 'undefined') {
  try {
    ...
    nativeRTCPeerConnection.prototype = iframe.contentWindow.RTCPeerConnection
...
function ChromeRTCPeerConnection(configuration) {
  ...
  /* eslint new-cap:0 */
  var peerConnection = new nativeRTCPeerConnection(newConfiguration);
  ...
  util.proxyProperties(nativeRTCPeerConnection.prototype, this, peerConnection);
}
...
ChromeRTCPeerConnection.prototype.addStream = function addStream(stream) {
  ...
  nativeRTCPeerConnection.prototype.addStream.apply(this._peerConnection, [stream]);
};
...
ChromeRTCPeerConnection.prototype.removeStream = function removeStream(stream) {
  ...
  nativeRTCPeerConnection.prototype.removeStream.apply(this._peerConnection, [stream]);
};
...
function maybeDispatchTrackEvents(peerConnection, mediaStreamTracks) {
  if ('ontrack' in nativeRTCPeerConnection.prototype) {
    return;
  }
